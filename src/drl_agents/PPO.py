...
model = PPO(
    ...
    env= vec_env, 
    learning_rate=1e-4,
    n_steps= 7500, #update every 15000 episodes
    n_epochs = 5, 
    clip_range =0.1, 
    verbose=1,
    gae_lambda=0.99, 
...
for i in range(1, 100):
    model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name="PPO", callback=callback, progress_bar= True)
